...
const uiConfig = {
    ...
    "Per_page": 16, // Set this to true to give us credit
};
...
<head>
  ...
  <link rel="stylesheet" href="//cdn.jsdelivr.net/gh/rocky2242/G_DriveIndex_V_4.1/css/darkmod.min.css">
  <style>${uiConfig.display_size ? '' : '.csize{display:none;}'}${uiConfig.display_time ? '' : '.cmtime{display:none;}'}</style>
    <link rel='stylesheet' href='https://fonts.googleapis.com/css?family=Poppins'>
  <script src="//cdn.jsdelivr.net/gh/rocky2242/G_DriveIndex_V_4.1/app.js"></script>
...
async function apiRequest(request, gd) {
	...
	if (path.substr(-1) == '/') {
		...
		let deferred_list_result = gd.list(path);
		...
		//console.log("JSON_deferred_list_result",JSON.stringify(list_result), option);
		return new Response(JSON.stringify(list_result));
	} else {
		let file = await gd.file(path);
		let range = request.headers.get('Range');
...
class googleDrive {
	...
	async list(path) {
		//console.log("_list_ page_token",page_token);
		...
		if (this.path_children_cache[path]
			...
			let child_obj = this.path_children_cache[path];
			return {
	...
	async _ls(parent) {
		//console.log("_ls_parent_or_pid", parent);
		if (parent == undefined) {
			return null;
		}
        const files = [];
		...
		params.orderBy = 'folder,modifiedTime desc';
		params.fields = "nextPageToken, files(id, name, mimeType, size , modifiedTime, thumbnailLink)";
		params.pageSize = 800;
	 ...
	 */
	async search(origin_keyword, page_token = null) {
		const types = CONSTS.gd_root_type;
	...
	async findPathId(path) {
		...
		//console.log("_return path",this.paths[path]);
