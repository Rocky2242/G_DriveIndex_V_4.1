...
function append_search_result_to_list(files) {
	...
	for (i in files) {
		...
		if (item['mimeType'] == 'application/vnd.google-apps.folder') {
			...
			html += `<div onclick="onSearchResultItemClick(this)" id="${item['id']}" gd-type="${item.mimeType}" class="col-12 col-sm-4 col-md-3 col-lg-3 col-xl-3">
                        <div class="card">
                             <img class="card-img-top" src="${item['thumbnailLink']}">
                                <div class="card-body">
                                <p class="card-text">${item.name}<br>Size:${item['size']}</p>
                                </div>
                         </div>
